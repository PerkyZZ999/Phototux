...
**Reset is not done until the shell has been told.** In the shipping host, `persist_workspace_visibility` is the single place that publishes a layout change: it applies the workspace to preferences, writes them, and emits the five properties QML binds the dock to (`panelVisibilityJson`, `dockTopologyJson`, `dockGroupsJson`, `workspaceFocusJson`, `activeWorkspacePresetId`). `reset_workspace` used to stop at the *preference* fields, which carry panel visibility but not dock topology, so an auto-hidden panel stayed hidden behind a toast reading "Workspace reset to Essentials" — the one control that is supposed to undo any layout mistake could not undo that one. `a_slot_that_changes_the_workspace_layout_republishes_it` in `phototux_ui` now reads the host as text and fails any `#[qslot]` that mutates the workspace without reaching that publication, directly or through `commit_workspace_op`; workspace *focus* and `active_preset_id` are the named exceptions, because neither is layout and each publishes its own property.
