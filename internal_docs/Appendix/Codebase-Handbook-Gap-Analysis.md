...
| **9** | `LayerMask::feather` is recorded but not rendered | The field is stored, clamped by `clamp_refine`, recorded in undo by `mask.set-attributes`, and bound to a QML property — and `feather` appears nowhere in `phototux_gpu`. Unlike density/contrast/shift it is a neighbourhood operation, so `LayerMask::coverage` cannot express it per sample. The engine already has `feather_mask_r8`, used for selections | Needs an R8 blur pipeline: the composite samples masks from an R8 array and `SeparableBlur` is RGBA-only, while the CPU route would mean a GPU readback per repack, which the interactive guardrails reserve for tests and degraded mode. The control is shown disabled and labelled meanwhile, so the UI no longer offers a slider that changes nothing |
