...
| Shipped detail (2026-09-04) | The strip's order is **the order tabs were opened**, held in `DocumentRegistry::order` and independent of which tab is active. It was previously derived from "active first, then the parked vector" — and parking pushes to the back of that vector, so switching shuffled twice: the clicked tab jumped to position 0 and the one left behind went to the end. Activating moves a document between the parked vector and the host's active slot and must not disturb the order; closing is the one path that removes a tab, through `forget`. |
| Shipped detail (2026-09-04) | Opening a file the session already holds **raises that tab**, as Photoshop does, instead of making a second one with its own history from which a save would silently lose the other's edits. "Already open" is answered by `SessionState::source_path` — the file a document was *loaded from*, whatever its format — which is deliberately not `document_path`, the place `Save` writes. The two agree only for a `.ptx`: a raster or PSD import has no `.ptx` yet, and writing one over the file it came from would destroy it. `source_path` is also what the file dialogs start from, so an imported PNG now opens the browser in its own folder. |
