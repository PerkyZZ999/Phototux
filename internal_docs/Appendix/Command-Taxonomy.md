...
| Filter / style | `filter.add-adjustment`, `filter.set-parameters`, `filter.add-effect`, `filter.set-gaussian-radius`, `filter.preview`, `filter.set-preview-params`, `filter.commit`, `filter.cancel-preview`, `effect.reorder`, `effect.set-enabled`, `style.add` |
