...
| T-045 | med | §1.3 | The tab strip reordered itself on every switch — the clicked tab jumped to position 0 and the one left behind went to the end — and opening a file already open made a second tab with its own history, from which a save silently lost the other's edits | Open three documents, click the leftmost; then Ctrl+O the file already open | **fixed** — `DocumentRegistry::order` holds the strip order independent of which tab is active, and `forget` removes a closed one. "Already open" is answered by the new `SessionState::source_path`, the file a document was loaded from, which is deliberately not `document_path`, the place Save writes |
| T-046 | med | §3 | One drag of a dock seam to the bottom of the screen made the panel above fill the dock and every group below it vanish — not collapsed, not scrollable, gone — while the Window menu still listed them as visible. `PanelResizeGrip` clamped at a constant 2000 and neither side subtracted what the stack below needed | Press the seam above the Navigator header and drag to the bottom | **fixed** — `panelMaxHeight` supplies the ceiling from the dock, reserving a header and a minimum body for every group below; `every_dock_seam_is_clamped_against_the_dock` pins the helper and the binding at all five seams, because a missing binding reads as 0 and falls back to the absolute bound silently |
| T-047 | low | §4.4 | The free-transform box was drawn unclipped: dragging a layer up painted its top edge and handles across the document tab strip, on top of the tab labels. A QML child paints outside its parent unless the parent clips, and the overlays are positioned from document coordinates | Free Transform, then drag a layer up and to the right | **fixed** — every document overlay lives in `canvasOverlays`, which clips to the viewport, with `PhototuxCanvas` outside it as a sibling. `canvas_overlays_are_clipped_and_the_canvas_is_not` pins both halves, because the tempting one-line version clips the zero-copy present too |
| T-048 | med | §7.2–7.3 | The Text and Shape tools threw away the click that created the layer: the text frame landed at the document origin, a thousand pixels from the pointer on a 1080p canvas, and every shape preset landed at its fraction of the document. The position never left QML — `CommandArgs::TextCreate` carried only the string | Press `t`, click at the bottom-right of the canvas | **fixed** — `TextCreate` carries the point and places the layer's translation, which is what `textOriginX`/`Y` publish; `ShapePreset::content_at` centres a preset's path on it. Both clamp into the document. The menu path keeps the unplaced preset: there is no pointer to honour there |
| T-049 | med | §7.3 | Path Edit asked the user to drag anchors it never drew — hit-testing is host-side, so the pointer found anchors the screen never showed, while the tool's own copy read "Drag anchors to move. Click empty to add". On a raster layer the first click was refused silently, because nothing ever created the first document path | Shape layer, press `A`; then a raster layer, click the canvas twice | **fixed** — `pathGeometryJson` publishes the anchors and `pathEditChrome` draws them at a constant screen size with the selected one filled; adding an anchor starts a path when there is none; `inspector.path` is registered for every layer, closed |
| T-050 | med | §1.4 | Image ▸ Color ▸ Convert to Display-P3 rewrote every layer's pixels on the GPU behind a warning and recorded nothing to take it back; Ctrl+Z walked past the conversion to whatever came before it | Open an image, Convert to Display-P3, Ctrl+Z | **fixed** — the pixel-rewriting branch records a `Transform` entry and `invoke_command` snapshots before the engine sees the command, withdrawing it on refusal. A `TransformSnapshot` carries the graph beside the pixels, so one entry covers both halves. The first attempt put the snapshot in the `convert_document_profile` slot, which the menu path does not use — verified by sampling the canvas across convert, undo and redo |
| T-051 | low | §5.1 | The transparency lock was state nothing set and nothing read: `LockFlags::alpha` round-tripped through `.ptx` and `layer.set-locks` accepted it, while no predicate mentioned it and no control could turn it on | Layers panel ▸ Lock row — there were only three buttons | **fixed** — `Layer::alpha_locked` carried on `BrushParams::preserve_alpha`, a paint pipeline with `ColorWrites::COLOR` on the GPU and a colour-only blend in the CPU reference, held together by a device fixture. Four icon toggles now, in Photoshop's order; `checkerboard` and `lock` added to the qrc |
