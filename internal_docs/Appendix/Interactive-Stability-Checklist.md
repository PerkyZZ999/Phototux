...
| T-034 | med | §2.4 | The command palette does not open — neither Ctrl+Shift+P nor Edit → Command Palette… raises it, with a document open and no modal in the way. Reproduces on a build without the host-request work, so it predates it | Ctrl+Shift+P, or Edit → Command Palette… | **open** — the host request reaches `dispatch_host_op`; the QML side calls `commandPaletteLoader.ensure().showPalette()` and nothing appears |
