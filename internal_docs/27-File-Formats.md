...
The shipping product native editable format is **`.ptx`** ([DR-026](Appendix/Decision-Register.md#dr-026--native-ptx-container-v1)). **Accepted encoding today:** writers emit **format version 2** typed chunks (`MANI` / `RASL` / `MASK` / `SRCE`) + whole-body CRC32; readers still open **v1** monolithic bodies. This specification also defines **target** behavior for sparse/tile-addressable resources and incremental save — those remain **SHOULD / Phase 5 Provisional**, not v1 MUSTs. Encoding improvements **MUST** remain versioned and backward-open for existing `.ptx` files where practical.

`SRCE` carries a smart object's pristine source pixels ([DR-032](Appendix/Decision-Register.md#dr-032--graph-kind-set-includes-smartobject)), which are distinct from the `RASL` raster of the same layer: `RASL` holds what is *on screen*, which for a smart object is the placed result, and the next placement is computed from the source instead. The manifest's `smart_asset_ids` maps layer id to asset id, exactly as `mask_asset_ids` does. Both families allocate from one asset-id space and a test holds them apart, because the assets come back in one flat map and a collision would let one silently replace the other. An older reader skips the chunk, and an older document simply has none.
