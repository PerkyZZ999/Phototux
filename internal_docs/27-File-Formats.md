...
The shipping product native editable format is **`.ptx`** ([DR-026](Appendix/Decision-Register.md#dr-026--native-ptx-container-v1)). This specification defines target behavior and invariants for evolution (chunking, integrity, sparse resources, incremental save). Encoding improvements **MUST** remain versioned and backward-open for existing `.ptx` files where practical. Measure huge sparse documents, incremental saves, recovery, unknown-data round trips, and corruption handling before freezing advanced strategies — not before treating `.ptx` as the product format.
