...
| `ThemedButton.qml`, `ThemedCheckBox.qml`, `ThemedComboBox.qml`, `ThemedSlider.qml`, `ThemedSpinBox.qml`, `ThemedTextField.qml`, `ThemedDialogHeader.qml` | Controls themed off `Theme.qml` rather than the Basic style. A bare `Button` / `CheckBox` / `ComboBox` / `Slider` / `SpinBox` / `TextField` fails `no_unstyled_controls_reach_the_user`: with no style configured the shell runs Basic, whose light palette is invisible on a developer profile with a Qt style set system-wide and obvious on a clean one. `ThemedButton` takes a `prominence` of `normal`, `primary` or `danger`, and `flat` for a run of secondary actions. |
