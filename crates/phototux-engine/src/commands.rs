...
impl SessionState {
    ...
    pub fn invoke(&mut self, id: &str, args: CommandArgs) -> Result<CommandEffects, CommandError> {
        ...
        match id {
            ...
            command_id::DOCUMENT_CANVAS_SIZE => self.cmd_document_canvas_size(args),
    ...
    fn cmd_document_canvas_size(
        ...
        if graph.size == size {
            return Err(CommandError::Rejected("the canvas is already that size"));
        }
        graph.size = size;
        graph.revision = graph.revision.wrapping_add(1);
        self.size = size;
        ...
        self.history.push_transform("Canvas Size", generation);
        self.zoom_to_fit();
        let mut effects = CommandEffects::document_edit(generation);
        effects.sync_selection = true;
        effects.sync_camera = true;
